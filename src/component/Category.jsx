        ...
        <div>
            <form className='flex justify-center' >
                ...
                <div className="relative w-11/12">
                    ...
                    <input type="search" id="default-search" className="block w-full p-3 pl-12 text-sm text-white rounded-2xl bg-gray-50  bg-input" placeholder="Sherlock Holmes" onChange={handleChange} required />
                </div>
            </form>
                ...
                <div ref={emblaRef} className='overflow-hidden'>
                    <ul className='flex text-l'>
                        {category.map((genre, index) => (
                            <li key={index} className='ml-2 mr-2 min-w-fit xl:text-xl hover:font-bold hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-t hover:from-orange_dark hover:to-orange_light'>
                                <button id={genre.id} onClick={handleClick}>{genre.name}</button>
                            </li>
                        ))}
                    </ul>
                </div>
